...
const Staff = () => {
  ...
  const custsearch = e => {
    ...
    axios
      .post(
        URLS.GetCancelledBookingsSearch + `${e.target.value}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const GetOrderFiliter = () => {
    ...
    axios
      .post(URLS.GetCancelledBookings, data, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        setusers(res.data.data)
                      ...
                      <tbody>
                        {lists.map((data, key) => (
                          <tr key={key}>
                            ...
                            <td>{data.cancellReason}</td>
                            <td>{data.status}</td>
                            <td>
                            {Roles.cancelledBookingsView  || Roles?.accessAll === true ?<> 
                              <Button
                                onClick={() => {
                                  Actinid1(data)
                                }}
