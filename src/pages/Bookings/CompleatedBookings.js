...
const Staff = () => {
  ...
  const custsearch = e => {
    ...
    axios
      .post(
        URLS.GetCompleatedBookingsSearch + `${e.target.value}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const GetOrderFiliter = () => {
    ...
    axios
      .post(URLS.GetCompleatedBookings, data, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
        setusers(res.data.data)
      ...
      <div className="page-content">
        <div className="container-fluid">
          <Breadcrumbs
            ...
            breadcrumbItem="Completed Bookings"
          />
          {filter ? (
            <>
              <Card>
                      ...
                      <tbody>
                        {lists.map((data, key) => (
                          <tr key={key}>
                            ...
                            <td>{data.status}</td>
                            <td>
                            {Roles.compeletedBookingsView  || Roles?.accessAll === true ?<> 
                              <Button
                                onClick={() => {
                                  Actinid1(data)
                                }}
