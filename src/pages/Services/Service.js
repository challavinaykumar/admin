...
const Banner = () => {
  ...
  const AddBanner = () => {
    ...
    axios
      .post(URLS.AddService, dataArray, {
  ...
  const EditBanner = () => {
    ...
    axios
      .put(URLS.UpdateService + formid, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
  ...
  const DeleteBanner = data => {
    ...
    var remid = data._id
    axios
      .delete(URLS.DeleteService + remid, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
  ...
  const searchAll = e => {
    ...
    axios
      .post(
        URLS.GetServiceSearch + `${e.target.value}`,
        {},
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            ...
            breadcrumbItem="Occasions"
          />
          <Row>
          {Roles.occassionsAdd  || Roles?.accessAll === true ? <>
            <Col md={4}>
              <Card>
                <CardHeader className="bg-white">
                  <CardTitle>Add Occasions</CardTitle>
                </CardHeader>
                <CardBody>
                  <Form
                    onSubmit={e => {
              ...
              <Col md={Roles.occassionsAdd  || Roles?.accessAll === true ?8:12}>
              <Card>
                <CardHeader className="bg-white">
                  <CardTitle>Occasions List</CardTitle>
                </CardHeader>
                <CardBody>
                  <div>
                        ...
                        <tbody>
                          {lists.map((data, key) => (
                            <tr key={key}>
                              ...
                              <td>
                              {Roles.occassionsEdit  || Roles?.accessAll === true ?<>
                                <Button
                                  onClick={() => {
                                    getpopup(data)
                                  }}
                                ...
                                {Roles.occassionsDelete  || Roles?.accessAll === true ?<>
                                <Button
                                  onClick={() => {
                                    manageDelete(data)
                                  }}
